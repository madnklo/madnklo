...
where $\bar{\mathcal{S}}^{(i,j)}_{k}$ is the partial fractioned Eikonal:
\begin{equation}
\bar{\mathcal{S}}^{(i,j)}_{k} = \frac{p_j \cdot p_k}{(p_i + p_j) \cdot p_k } \mathcal{S}^{(i,j)}_{k}.
\end{equation}
One can understand the identification of the two colour correlators explicitly by inspecting the expanded expression of the colour correlators C.3 and C.4 of sect.~\ref{NNLOConventions}.

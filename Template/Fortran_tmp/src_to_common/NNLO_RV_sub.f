      subroutine nnlo_rv_sub(s,v,mk2,ml2,mu,ccrnLO,I1NNLO)
      implicit none
      include 'coupl.inc'
      include 'math.inc'
      double precision s,v,yp
      double precision mk2,ml2,mu
      double precision I1NNLO,ddilog,CCRNLO,Q2
      ...
      if(alpha.lt.beta.or.alpha.gt.1d0.or.beta.lt.0d0)then
         write(*,*)'Wrong alpha, beta, values in NNLO_RV_sub'
         write(*,*)alpha,beta
         stop
      endif
...
c$$$     $     dlog(lam**2/(mk2*ml2*Q2*s)) - dlog(s/mu**2) )
      ...
      I1NNLO = - 0.5d0*CCRNLO*(8d0+(4d0*mk2*dlog((1d0-beta)/(1d0+beta)))/dsqrt(lam)

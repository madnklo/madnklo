...
c$$$      if(ISUM_HEL.ne.0) then
c$$$        call sample_get_discrete_x(wgt,hel_picked,iconfig,'Helicity')
c$$$      endif
      end
...
c     At the moment we just consider an e+ e- collider
c     TODO: to implement xbj sampling for initial state hadrons 4/7/23
...
c$$$               p1(j,i) = p(j,i)
c$$$            enddo
c$$$c            write(*,'(i2,4e15.5)') i,(p(j,i),j=0,3)
c$$$         enddo
c$$$c         call fill_invarients(nfinal,p1,stot,xx)
c$$$         if (abs(lpp(1)) .ge. 1 .and. abs(lpp(1)) .ge. 1) then
c$$$            xx(invar)=x(ndim)
c$$$            xx(invar-1)=x(ndim-1)
c$$$         elseif (abs(lpp(1)) .ge. 1 .or. abs(lpp(1)) .ge. 1) then
c$$$            xx(invar)=x(ndim)
c$$$         endif
c$$$c
c$$$c     The next lines take care of redetermining the wgt based on all
c$$$c     allowed configurations
c$$$c
c$$$c         if (passcuts(p(0,1))) then
c$$$         if (.true.) then
c$$$c            write(*,'(A,5e14.4)') 'Orig',jac,pswgt,wgt,jac*pswgt*wgt
c$$$
c$$$            tprb=0d0
...
c$$$
c$$$c
c$$$c     I'll skip this if there is only one configuration
c$$$c
c$$$
c$$$               if (jac .lt. 0) then
c$$$                  write(*,*) 'Error did not find wgt ',jconfig,jac
c$$$               endif
c$$$
c$$$c               write(*,'(a,12f6.2)') 'final',(x(i),i=1,ndim)
c$$$
c$$$c
c$$$c     Now need to set up the x(i)'s
c$$$c
c$$$               do i=1,ndim
c$$$                  if (minvar(i,jconfig) .ne. 0) then
c$$$c                    write(*,*)'filling',minvar(i,jconfig),i,jconfig,x(i)
c$$$                     xx(minvar(i,jconfig))=x(i)
c$$$                  endif
c$$$               enddo
c$$$c
c$$$c     Here is where we put in something for pp etc. Now lets just use 1
c$$$c
c$$$c               if (pp) then
c$$$c                  xx(3*nbranch-3,jconfig)= x
c$$$
c$$$
c$$$               if (maxcfig-mincfig .gt. 0) then
c$$$               
c$$$                  pwgt(jconfig)=max(sjac*jac*pswgt*wgt,1d-99)
c$$$                  prb(jconfig,jpnt,jplace)=1d0/pwgt(jconfig)
c$$$                  tprb = tprb + prb(jconfig,jpnt,jplace)*alpha(jconfig)
c$$$               else
c$$$                  pwgt(1)=max(sjac*jac*pswgt*wgt,1d-99)
c$$$                  prb(1,jpnt,jplace)=1d0/pwgt(1)
c$$$                  tprb = tprb + prb(1,jpnt,jplace)
c$$$               endif
c$$$c               write(*,'(A,5e14.4)') 'Finl',jac,pswgt,wgt,jac*pswgt*wgt
c$$$
c$$$
c$$$
c$$$            enddo
...
c$$$
c$$$         else
c$$$            jac=-1d0              !Didn't pass the cuts.
c$$$         endif
c$$$         if (jac .lt. 0) then
c$$$            p1(0,1) = -999
c$$$            return
c$$$         endif
c$$$c
c$$$c     Flux factor and pi's from delta functions
c$$$c
c$$$         flux  = 1d0 /(2.D0*SQRT(LAMBDA(s(-nbranch),m(1)**2,m(2)**2)))
c$$$         flux  = flux / (2d0*pi)**(3 * nfinal - 4)
c$$$c
c$$$c     Make sure all the invarients are reasonable
c$$$c         
c$$$
c$$$         do i=1,invar
c$$$            x(i)=xx(i)
c$$$            if (x(i) .gt. 1d0) then
c$$$               write(*,'(a,i6,e15.4)') 'Error x>1',i,x(i)
c$$$            endif
c$$$            fprb(i,jpnt,jplace)=0d0
c$$$         enddo
c$$$
c$$$c
c$$$c     Determine fractional probs from the different configs this
c$$$c     allows me to only include points which were contributed
c$$$c     by the projection onto that invarient.
c$$$c
c$$$c
c$$$c     Now normalize fprb
c$$$c
c$$$         do i=1,invar
c$$$c            if (fprb(i,jpnt,jplace) .eq. 0d0) then
c$$$            if (tprb .eq. 0d0) then
c$$$               fprb(i,jpnt,jplace)=1d0
c$$$            else
c$$$               fprb(i,jpnt,jplace)=fprb(i,jpnt,jplace)/tprb
c$$$            endif
c$$$         enddo
c$$$         
c$$$         
c$$$c         write(123,'(2i6,1e15.5)') jpnt,jplace
c$$$c         write(123,'(5e15.9)') (fprb(i,jpnt,jplace),i=1,invar) 
...
c$$$         endif
c$$$      else                    !Failed cuts
c$$$c         write(*,*) 'Failed cuts'
c$$$         p1(0,1)=-99           
c$$$      endif
c$$$c
c$$$c     comment out everything funny here
c$$$c
c$$$      endif
      end
...
c
         tmin=-stot
         tmax= stot
c         call sample_get_x(wgt,x(nbranch+(-ibranch-1)*2),
c     &        nbranch+(-ibranch-1)*2,iconfig,0d0,1d0)

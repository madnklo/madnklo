...
c     TODO: check what the following lines actually do

              
c$$$              if (xo.eq.0d0.and.MMJJ.gt.0d0) then
c$$$                 njet = 0
c$$$                 do k =1,2
c$$$                    if (iforest(k,i,iconfig).gt.0)then
c$$$                      if (is_a_j(iforest(k,i,iconfig))) njet = njet + 1
c$$$                    endif
c$$$                 enddo
c$$$                 if (njet.eq.1) then
c$$$                    xo = (MMJJ/1d2)**2/stot
c$$$                 else if (njet.eq.2) then
c$$$                    xo = (MMJJ * 0.8)**2/stot
c$$$                 endif
c$$$              endif
c$$$              if (xo.eq.0d0) xo=1d0/stot
              if (xo.eq.0d0) xo=min(1d1/stot,stot/50d0,0.5d0)

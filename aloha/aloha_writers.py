...
#####################################################################
#gl
class WriteALOHAUser(WriteALOHA): 
    """ Generic writing functions """ 
    
    def __init__(self, abstract_routine, dirpath, user_prefix):
        if aloha.loop_mode:
            self.momentum_size = 4
        else:
            self.momentum_size = 2
...
class ALOHAWriterForFortranUser(WriteALOHAUser): 
    """routines for writing out Fortran"""
    ...
    def write_combined(self, lor_names, mode='self', offshell=None):
        ...
        if offshell is None:
            ...
            sym = None
        #name = 'B_' + combine_name(self.routine.name, lor_names, offshell, self.tag)
        name = self.user_prefix + combine_name(self.routine.name, lor_names, offshell, self.tag)
        self.name = name
        # write head - momenta - body - foot
        text = StringIO()
        routine = StringIO()
        ...
        for i, name in enumerate(lor_list):
            #data['name'] = 'B_' + name
            data['name'] = self.user_prefix + name
            data['coup'] = 'COUP%d' % (i+1)
            if i == 0:
                if  not offshell: 
                    data['out'] = 'vertex'
...
class ALOHAWriterForFortranQPUser(QP, ALOHAWriterForFortranUser):
    
    def __init__(self, *arg):
        return ALOHAWriterForFortranUser.__init__(self, *arg)
    
class ALOHAWriterForFortranLoopUser(ALOHAWriterForFortranUser): 
    """routines for writing out Fortran"""

    def __init__(self, abstract_routine, dirpath, user_prefix):
        
        ALOHAWriterForFortranUser.__init__(self, abstract_routine, dirpath, user_prefix)
        # position of the outgoing in particle list        
        self.l_id = [int(c[1:]) for c in abstract_routine.tag if c[0] == 'L'][0] 
        self.l_helas_id = self.l_id   # expected position for the argument list
        if 'C%s' %((self.l_id + 1) // 2) in abstract_routine.tag:
...
class ALOHAWriterForFortranLoopQPUser(QP, ALOHAWriterForFortranLoopUser): 
    """routines for writing out Fortran"""     
    
    def __init__(self, *arg):
        return ALOHAWriterForFortranLoopUser.__init__(self, *arg)   


class ALOHAWriterForCPPUser(WriteALOHAUser): 
    """Routines for writing out helicity amplitudes as C++ .h and .cc files."""
    ...
    def write(self, **opt):
        ...
        cc_text = WriteALOHAUser.write(self, **opt)
        h_text = self.get_h_text()
...
class ALOHAWriterForGPUUser(ALOHAWriterForCPPUser):
    ...
    def get_header_txt(self, name=None, couplings=None, mode=''):
        ...
        if not 'is_h' in mode:
            text.write('__device__=__forceinclude__\n')
        text.write(ALOHAWriterForCPPUser.get_header_txt(self, name, couplings, mode))
        return text.getvalue()
...
class ALOHAWriterForPythonUser(WriteALOHAUser):
    """ A class for returning a file/a string for python evaluation """
    ...
    @staticmethod
    def change_number_format(obj, pure_complex=''):
        change_number_format = ALOHAWriterForPythonUser.change_number_format
        if obj.real == 0 and obj.imag:
            if int(obj.imag) == obj.imag: 
                return '%ij' % obj.imag
            else:
...
#gl
class WriterFactoryUser(object):

    def __new__(cls, data, language, outputdir, tags, user_prefix):
        language = language.lower()
        if isinstance(data.expr, aloha_lib.SplitCoefficient):
            assert language == 'fortran'
            if 'MP' in tags:
                return ALOHAWriterForFortranLoopQPUser(data, outputdir, user_prefix)
            else:
                return ALOHAWriterForFortranLoopUser(data, outputdir, user_prefix)
        if language == 'fortran':
            if 'MP' in tags:
                return ALOHAWriterForFortranQPUser(data, outputdir, user_prefix)
            else: 
                return ALOHAWriterForFortranUser(data, outputdir, user_prefix)
        elif language == 'python':
            return ALOHAWriterForPythonUser(data, outputdir, user_prefix)
        elif language == 'cpp':
            return ALOHAWriterForCPPUser(data, outputdir, user_prefix)
        elif language == 'gpu':
            return ALOHAWriterForGPUUser(data, outputdir, user_prefix)
        else:
            raise Exception, 'Unknown output format'

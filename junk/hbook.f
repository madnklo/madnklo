...
c        CALL MBKWRN('MNORM')
    ...
    1 HIST(N,I)=HIST(N,I)*Y
      ...
      SUBROUTINE MPRINT(N,resc)
      IMPLICIT NONE
      INCLUDE 'hbook.inc'
      INTEGER N,J
      CHARACTER CTIME*10,CDATE*10
      CHARACTER*50 TITLE3(NMB)
...
c      WRITE(98,'(a)')'# OBSERVABLE = '//TITLE(N)
c      WRITE(98,15) HAVG(N),HSIG(N),HINT(N)
c      WRITE(98,20) IENT(N),IUSCORE(N),IOSCORE(N)
      TITLE3(N)='"'//trim(TITLE(N))//' "'
      IF(N.EQ.1)WRITE(98,'(a)')'##& xmin & xmax & central value & dy'
      WRITE(98,*)
      WRITE(98,11)NBIN(N),TITLE3(N)
      DO 1 J=1,NBIN(N)
c      IF(HIST(N,J).EQ.0.) GO TO 1
      WRITE(98,12)
     &   XHIS(N,J)-HDEL(N)/2d0,XHIS(N,J)+HDEL(N)/2d0,HIST(N,J)*resc,0d0
    1 CONTINUE
      WRITE(98,'(a)')'<\histogram>'
    7 FORMAT('# HIST = ',I3,'   20',I2,'-',I2,'-',I2,1X,A5)
   10 FORMAT(4X,2E10.3)
   11 FORMAT('<histogram> ',I4,1X,A30)
   12 FORMAT(4(2x,e14.7))
   15 FORMAT('# AVG = ',E13.6,5X,' RMS = ',E13.6,5X' INT = ',E13.6)
   20 FORMAT('# ENT = ',I10,5X,'UND = ',I10,5X,'OVR = ',I10)
      END

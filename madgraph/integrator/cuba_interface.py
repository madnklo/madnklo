###########################################################################################
# This file is a slightly modified copy of the __init__.py from the Python module
...
integrand_type_generic = ctypes.CFUNCTYPE(c_int, POINTER(c_int), 
  POINTER(c_double), POINTER(c_int), POINTER(c_double), c_void_p)
integrand_type_vegas = ctypes.CFUNCTYPE(c_int, POINTER(c_int), 
  POINTER(c_double), POINTER(c_int), POINTER(c_double), c_void_p, 
...
def wrap_integrand_generic(integrand):
  use_raw_callback = isinstance(integrand, ctypes._CFuncPtr)
  if use_raw_callback:
    return integrand
  else:
    return integrand_type_generic(integrand)

def wrap_integrand_vegas(integrand):
  use_raw_callback = isinstance(integrand, ctypes._CFuncPtr)
  if use_raw_callback:
    return integrand
  else:
    return integrand_type_vegas(integrand)
  
def set_cuba_nb_core(n_cpus, n_cpus_max=1000):
    """ Specify n_cpus and n_accel last."""
    lib.cubacores(n_cpus, n_cpus_max)
...
def Suave(integrand, ndim, nnew=1000, nmin=2, flatness=50., userdata=NULL, 
  ...
  *flatness*: the parameter p in Eq. (1), i.e. the type of norm
  ...
  lib.Suave(ndim, ncomp, wrap_integrand_generic(integrand), userdata,
 ...
 *peakfinder*: the peak-finder subroutine. This subroutine is called
  ...
  lib.Divonne(ndim, ncomp, wrap_integrand_generic(integrand), userdata,
    c_int(nvec), c_double(epsrel), c_double(epsabs), verbose, seed,
...
def Cuhre(integrand, ndim, 
  ...
  *key* chooses the basic integration rule:
  ...
  lib.Cuhre(ndim, ncomp, wrap_integrand_generic(integrand), userdata,

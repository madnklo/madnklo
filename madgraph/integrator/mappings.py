...
#from madgraph.iolibs.template_files.subtraction.subtraction_schemes.torino.factors_and_cuts import extra_index, indice
import madgraph.iolibs.template_files.subtraction.subtraction_schemes.torino.factors_and_cuts as factors_and_cuts
...
class SoftVsFinalPureRescalingMapping(ElementaryMappingSoft):
    ...
    def is_valid_structure(cls, singular_structure):
        ...
        #logger1.info('singular_structure.legs: ' + str(singular_structure.legs))
...
# #            pa = new_PS_point[ia]
# #            pb = tmp_new_PS_point[ib]
# #            pc = tmp_new_PS_point[ic]
#         else:
#             pa = new_PS_point[ia]
#             pb = new_PS_point[ib]
#             pc = new_PS_point[ic]
...
#             # the jacobian should be 1
# #            jacobian = 1. / (sac + sbc) / sabc
# #            if jacobian > 1.:
# #                jacobian = 1.

# #            jacobian = ( sabc / (sac + sbc) ) * ( sbc / (sac + sbc) )
# #            jacobian = 1.
...
#     # we never have this case since we sum over (l < k) in soft ct
...
#             x = 1. - (sab + sac) / sbc
#             v = sab / (sab + sac)
#             #logger1.info('ia : ' + str(ia) + '; ' + 'ib: ' + str(ib) + '; ' + 'ic: ' + str(ic))
...
#         # for i_fs in range(1,len(new_PS_point)+1):
#         #     if  i_fs != ia:
#         #         logger1.info('Pre - Remapped momenta' + '= ' + str(i_fs) + str(new_PS_point[i_fs]))

#             if ib==2 and new_PS_point[ib][3] < 0.:
#                 #logger1.info('new_PS_point[ib][3] : ' + str(new_PS_point[ib][3]))
#                 newtmp_ib = new_PS_point[ib]
#                 newtmp_ic = new_PS_point[ic]
#                 new_PS_point[ib] = newtmp_ic
#                 new_PS_point[ic] = newtmp_ib
#                 #logger1.info(str(ib) + '; ' + str(ic))
#                 newtmp_ib = ib
#                 newtmp_ic = ic
#                 ib = newtmp_ic
#                 ic = newtmp_ib
...
#         #logger1.info('Final return = ' + str(ia) + ' , ' + str(ib) + ' , ' + str(ic))
...
#        logger1.info('child_1: ' + str(singular_structure.substructures[0].legs[0].n))
#        logger1.info('child_2: ' + str(singular_structure.substructures[0].legs[1].n))
#        logger1.info('Recoiler: ' + str(singular_structure.legs[0].n))
...
#            jacobian = 1.
        ...
        # the jacobian should be 1
        jacobian = 1.
        #logger1.info('jacobian :' + str(jacobian))

...
class integrated_NLO_FF_QCD_collinear_qqx(integrated_NLO_FF_QCD_current):
    ...
    def evaluate_integrated_current(self,  current, 
        ...
        #Set the upper bound on the virtuality
        # Get the collinear object that's being subtracted ('C(i,j)')
        Cij = current.get('singular_structure').substructures[0]
        # Get the legs that go collinear
        pipj = [l.n for l in Cij.legs]
        # Use the leg number map to find the mapped leg corresponding to the collinear pair
        pij_tilde = PS_point[leg_numbers_map.inv[frozenset(pipj)]]
        yijtildeQ = 2 * pij_tilde.dot(Q)/Q_square

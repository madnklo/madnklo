...
class GeneralCurrent(utils.VirtualCurrentImplementation):
    ...
    @classmethod
    def match_current(cls, current, template_current, model):
        ...
        for a_template_current in all_template_currents:
            ...
            if current_type in ['IntegratedCurrent', 'BeamCurrent', 'IntegratedBeamCurrent']:

                found_a_mismatch = False
                for beam_property in ['beam_type', 'beam_PDGs', 'distribution_type']:
                    # Note that when setting None for this property in the template current, this implies that
                    # it should match all variants.
                    if a_template_current[beam_property] not in [None, current[beam_property]]:
                        found_a_mismatch = True
                        break
                    else:
                ...
                matching_current_properties['active_fermions'] = sorted(list(set(abs(pdg) for pdg in current['beam_PDGs'] if
                                                                    model.get_particle(pdg).get('spin') == 2)))
                matching_current_properties['NF'] = len([1 for pdg in matching_current_properties['active_fermions'] if
            ...
            leg_numbers_map = cls.match_singular_structure(
                        current_type['singular_structure'], a_template_current['singular_structure'], model)
    ...
    def evaluate_subtraction_current( self,
        ...
        hel_config=None, mu_r = None, mu_fs=(None,None), xis=(None,None), Q=None, track_leg_numbers = True,
        allowed_backward_evolved_flavors=('ALL','ALL'), **opts ):
        ...
        # Default the rescalings at None. Beam currents will overwrite it.

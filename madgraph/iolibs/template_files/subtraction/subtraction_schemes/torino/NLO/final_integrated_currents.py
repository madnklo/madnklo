...
logger.info("Damping factors = " + str(factors_and_cuts.damping_factors))
...
class QCD_integrated_TRN_C_FgFg(general_current.GeneralCurrent):
    ...
    def kernel(self, evaluation, all_steps_info, global_variables):
        ...
        double = 0.
        single = overall
        finite = overall * (5. / 3. + factors_and_cuts.A2(factors_and_cuts.beta_FF))
        #finite = overall * (8. / 3. - math.log(etapr))

        #from the soft part
        #overall = self.CA
        #double += overall
        #single += overall * ( 2. * factors_and_cuts.A2(factors_and_cuts.alpha)  )
        #finite += overall * ( (math.pi**2 / 12.) + 2. * (factors_and_cuts.A2(factors_and_cuts.alpha))**2 - 4. * factors_and_cuts.polygamma(factors_and_cuts.alpha) )
...
class QCD_integrated_TRN_C_FgFq(general_current.GeneralCurrent):
    ...
    def kernel(self, evaluation, all_steps_info, global_variables):
        ...
        double = 0.
        single = overall
        finite = overall * (1. + factors_and_cuts.A2(factors_and_cuts.beta_FF))
    ...
    def soft_kernel(self, evaluation, colored_partons, all_steps_info, global_variables):
        ...
        for i, a in enumerate(colored_partons):
            ...
            for b in colored_partons[i:]:
                ...
                #single = 0.
                #finite = 0.
                #finite = - 6. + 7./2. * (math.pi**2 / 6.) + math.log(etaab) * (2. - math.log(etaab)/2.)

                double = - 1.
                single = - 2. * factors_and_cuts.A2(factors_and_cuts.alpha) 
                finite = - (math.pi**2 / 12.) - 2. * (factors_and_cuts.A2(factors_and_cuts.alpha))**2 + 4. * factors_and_cuts.polygamma(factors_and_cuts.alpha)
...
class QCD_integrated_TRN_CS_FgFq(general_current.GeneralCurrent):
    ...
    def kernel(self, evaluation, all_steps_info, global_variables):
        ...
        overall = - 2. * self.CF
        double = 0.
        single = overall * (1. - factors_and_cuts.A2(factors_and_cuts.alpha))
        #logger.info("single from CS = " + str(single))
        finite = overall * (2.  - (math.pi**2 / 4.) + factors_and_cuts.A2(factors_and_cuts.beta_FF)) * (1. - factors_and_cuts.A2(factors_and_cuts.alpha)) - (factors_and_cuts.A2(factors_and_cuts.alpha))**2 / 2. + (3./2.) * factors_and_cuts.polygamma(factors_and_cuts.alpha)

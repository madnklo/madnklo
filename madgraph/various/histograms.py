...
class Histogram(object):
    ...
    def MULTIPLY(wgtsA, wgtsB):
        ...
        for label, wgt in list(wgtsA.items()):    
            if label=='stat_error':
                continue
...
#                    print "for label", label
    ...
    def SINGLEHISTO_OPERATION(wgts, wgt_operation, stat_error_operation):
        ...
        new_wgts = {}
        for label, wgt in list(wgts.items()):
            if label!='stat_error':
...
# ===========  END HELPER FUNCTIONS ===========
        ...
        if consider_reweights!='ALL':
            ...
            for wgt_position, wgt_labels in list(selected_weights.items()):
...
'%s PDF=%s' % (title,pdf[j].replace('_','\_'))))
        ...
        for one_plot in uncertainty_plot_lines:
            for uncertainty_type, lines in list(one_plot.items()):
    ...
    for arg in sys.argv:
        ...
        if opt=='--variations':
            uncertainties=[variation_type_map[type] for type in eval(value,
                         dict([(key,key) for key in list(variation_type_map.keys())]+
                                          [('all',list(variation_type_map.keys()))]))]
        if opt=='--band':
            use_band=[variation_type_map[type] for type in eval(value,
                         dict([(key,key) for key in list(variation_type_map.keys())]+
        [('all',[type for type in list(variation_type_map.keys()) if type!='stat'])]))]

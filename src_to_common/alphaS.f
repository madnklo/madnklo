      FUNCTION ALPHA_QCD(asmz,nloop,Q)
c
c     Evaluation of strong coupling constant alpha_QCD
c     Author: R.K. Ellis
c
c     q -- scale at which alpha_QCD is to be evaluated
c
c-- common block alfas.inc
c     asmz -- value of alpha_QCD at the mass of the Z-boson
...
C-----------------------------------------------------------------------------
      ...
      IMPLICIT NONE
c
      DOUBLE PRECISION Q,T,AMZ0,AMB,AMC,ALPHA_QCD
      DOUBLE PRECISION ASMZ,AS_OUT
...
C
      ...
      IF (Q .LE. 0D0) THEN 
         WRITE(6,*) 'q .le. 0 in alpha_QCD'
         WRITE(6,*) 'q= ',Q
         STOP
      ENDIF
      IF (asmz .LE. 0D0) THEN 
         WRITE(6,*) 'asmz .le. 0 in alpha_QCD',asmz
c         WRITE(6,*) 'continue with asmz=0.1185'
         STOP
         asmz=0.1185D0
      ENDIF
      IF (CMASS .LE. 0.3D0) THEN 
         WRITE(6,*) 'cmass .le. 0.3GeV in alpha_QCD',CMASS
c         WRITE(6,*) 'continue with cmass=1.5GeV?'
         STOP
         CMASS=1.42D0
      ENDIF
      IF (BMASS .LE. 0D0) THEN 
         WRITE(6,*) 'bmass .le. 0 in alpha_QCD',BMASS
...
c---  with a precision given by tol.
      ...
      INTEGER NLOOP,NF
      DOUBLE PRECISION T,A_IN,A_OUT,AS,TOL,F2,F3,F,FP,DELTA
      DOUBLE PRECISION B0(3:5),C1(3:5),C2(3:5),DEL(3:5)
      PARAMETER(TOL=5.D-4)
C---     B0=(11.-2.*NF/3.)/4./PI

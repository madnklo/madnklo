...
c
 ...
 999  ierr=1
      ...
      double precision function M2_S2(i,xs,xp,wgt,ZSoft,xj,xjB,XX,nit,extra,ierr)
c     single-soft limit S_(i) * Zsoft
c     it returns 0 if i is not a gluon
      implicit none
      include 'nexternal.inc'
      ...
      INCLUDE 'input.inc'
      INCLUDE 'run.inc'      
      integer i,l,m,lb,mb,ierr,nit,idum,ia
      double precision pref,M2tmp,wgt,wgtpl,Zsoft,xj,xjB,xjCS,xx(3),ddum
      double precision xs(nexternal,nexternal),xsb(nexternal-1,nexternal-1)
      double precision BLO,ccBLO,extra
      double precision xp(0:3,nexternal),xpb(0:3,nexternal-1)
      double precision sil,sim,slm,y,z,x,damp
...
c     underlying Born configuration is remapped
          iA = 1                ! default azimuth for NLO
              call phase_space_CS(xx,i,l,m,ia,xp,xpb,nexternal,leg_PDGs,'S',xjCS)
            if(xjCS.eq.0d0)goto 999
            call invariants_from_p(xp,nexternal,xsb,ierr)
            if(ierr.eq.1)goto 999
            call get_Z_NLO(xs,ddum,ddum,ISEC,JSEC,Zsoft,'S',ierr)
            IF(IERR.EQ.1)GOTO 999
c
c     invariant quantities
            sil=xs(i,l)
...
c     eikonal
            ccBLO = %(proc_prefix_S)s_GET_CCBLO(lb,mb)
            M2tmp=ccBLO*2d0*slm/(sil*sim) * Zsoft * xjB * xjCS
c
c     Including correct multiplicity factor
            M2tmp = M2tmp*dble(%(proc_prefix_S)s_den)/dble(%(proc_prefix_real)s_den)
c
c     damping factors
          if(alpha.ne.0d0)then
             write(*,*)'Implement damping factors in M2_S2!'
             stop
          endif
            if(m.gt.2.and.l.gt.2)then
               y=sil/(sil+sim+slm)
            ...
            M2tmp=M2tmp*damp
            M2_S2=M2_S2+pref*M2tmp*Zsoft*extra
c
         enddo 
      enddo
c
c     apply flavour factor
      M2_S2 = M2_s2 * %(proc_prefix_real)s_fl_factor
c
c     plot
      wgtpl=-M2_S2*wgt/nit
      if(doplot)call histo_fill(xpb,xsb,nexternal-1,wgtpl)
c
c     sanity check
      if(abs(M2_S2).ge.huge(1d0).or.isnan(M2_S2))then
         write(77,*)'Exception caught in M2_S2',M2_S2
         goto 999
      endif
c
      return

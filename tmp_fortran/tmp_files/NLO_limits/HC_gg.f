...
c     collinear limit C_(ia,ib) * Wcollinear - S_(ia)C_(ia,ib)
      ...
      double precision pref,M2_C_gg,M2_SC_gg,wgt,wgtpl,wgt_chan,xj,extra
...
c     
c     checks
      if(leg_pdgs(ia).ne.21.or.leg_pdgs(ib).ne.21.or.mapped_labels(ib).ne.21)then
      ...
      if(.not.(ia.eq.isec.and.ib.eq.jsec.and.ir.eq.iref))then
         write(*,*)'Wrong indices in M2_HC_gg',ia,ib,ir,isec,jsec,iref
         stop
      endif
c
c     possible cuts

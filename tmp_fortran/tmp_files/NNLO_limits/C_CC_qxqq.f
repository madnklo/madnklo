...
c     collinear triple-collinear limit C_(ia,ib)C_(ia,ib,ic) [WC_CC]
...
c     initial checks and label assignment
      if(lsec.eq.0)then
         ...
         elseif((i.eq.jsec.and.j.eq.ksec).or.(i.eq.ksec.and.j.eq.jsec)) then
            ...
            write(*,*)'Wrong indices 1 in M2_C_CC_qxqq'
         ...
         elseif((i.eq.ksec.and.j.eq.lsec).or.(i.eq.lsec.and.j.eq.ksec)) then
            ...
            write(*,*)'Wrong indices 2 in M2_C_CC_qxqq'
...
c     safety check
      ...
      IF(SIJ.LE.0D0.OR.SIR.LE.0d0.OR.SJR.LE.0D0)THEN
        WRITE(77,*)'Inaccuracy 1 in M2_C_CC_qxqq',SIJ,SIR,SJR
        GOTO 999
      ENDIF
      zi   = sir/(sir+sjr)
      zj   = 1d0-zi
...
c     apply flavour factor
      M2_C_CC_qxqq=M2_C_CC_qxqq*%(proc_prefix_rr)s_fl_factor
c
c     plot

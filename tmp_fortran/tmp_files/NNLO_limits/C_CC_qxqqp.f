...
c     collinear triple-collinear limit C_(ia,ib)C_(ia,ib,ic)
...
c     initial checks and label assignment
      if(lsec.eq.0)then
         ...
         elseif((i.eq.jsec.and.j.eq.ksec).or.(i.eq.ksec.and.j.eq.jsec)) then
            ...
            write(*,*)'Wrong indices 1 in M2_C_CC_qxqqp'
         ...
         elseif((i.eq.ksec.and.j.eq.lsec).or.(i.eq.lsec.and.j.eq.ksec)) then
            ...
            write(*,*)'Wrong indices 2 in M2_C_CC_qxqqp'
...
c     Call Born
      ...
      BLO = ANS(0)
c
c     Formula (C.39) of 2212.11190v2 TODO: complete kernel (_y)
...
c     apply flavour factor
      M2_C_CC_qxqqp=M2_C_CC_qxqqp*%(proc_prefix_rr)s_fl_factor
c
c     plot

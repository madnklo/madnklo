      ...
      function M2_C_RV_qqx(ia,ib,ir,xs,xp,xsb,xpb,wgt,xj,nit,extra,wgt_chan,ierr)
...
c     for sector (ia,ib)
      ...
      INCLUDE 'input.inc'
      INCLUDE 'run.inc'
      double precision M2_C_RV_qqx(-2:0)
      ...
      double precision %(proc_prefix_C_RV_qqx)s_GET_KKBLO
      double precision %(proc_prefix_C_RV_qqx)s_GET_KKVLO
...
c     set logical doplot
      ...
      integer %(proc_prefix_C_RV_qqx)s_den
      common/%(proc_prefix_C_RV_qqx)s_iden/%(proc_prefix_C_RV_qqx)s_den
...
c     checks
      if(.not.(abs(leg_pdgs(ia)).le.6.and.leg_pdgs(ia)+leg_pdgs(ib).eq.0))then
         write(*,*)'Wrong pdgs in M2_C_RV_qqx',leg_pdgs(ia),leg_pdgs(ib)
         stop
      endif
      if(.not.(ia.eq.isec.and.ib.eq.jsec))then
         write(*,*)'Wrong indices in M2_C_RV_qqx',ia,ib,isec,jsec
         stop
      endif
c
c     possible cuts
...
c     safety check
      if(sab.le.0d0.or.sar+sbr.le.0d0.or.x.le.0d0.or.x.ge.1d0)then
         write(77,*)'Inaccuracy 1 in M2_C_RV_qqx',sab,sar+sbr,x
         goto 999
      endif
c
c     call Born
...
c
      ...
      if(underlying_leg_pdgs(parent_leg).ne.21)then
         write(*,*)'wrong parent label in M2_C_RV_qqx'
         write(*,*)ib,parent_leg,mapped_labels(ib),underlying_leg_pdgs(parent_leg)
         stop
      endif
c
c     TODO: improve ktmuktnuBmunu / kt^2
      KKBLO = %(proc_prefix_C_RV_qqx)s_GET_KKBLO(parent_leg,xpb_to_ME,kt)
      KKVLO = %(proc_prefix_C_RV_qqx)s_GET_KKVLO(parent_leg,xpb_to_ME,kt)
      M2TMP0 = TR*(BLO-4d0/sab*KKBLO)

      M2TMP(-2:0) = M2TMP(-2:0) + TR*(BLO-4d0/sab*KKBLO)
      M2TMP(-2) = M2TMP(-2) + alphas/2d0/pi*M2TMP0*(CA-2d0*CF)
      M2TMP(-1) = M2TMP(-1) + alphas/2d0/pi*(M2TMP0*((2d0*CF-CA)*logab+CA*log(x*(1d0-x))-beta0/2d0)+(4d0*x*(1d0-x)/sab*KKBLO-BLO)*TR*(beta0-3d0*CF))
      M2TMP( 0) = M2TMP( 0) + alphas/2d0/pi*(M2TMP0*((CA-2d0*CF)*(7d0*zeta2-logab**2)/2d0+CA*(-logab*log(x*(1d0-x))+ddilog(-(1d0-x)/x)+ddilog(-x/(1d0-x))))+(4d0*x*(1d0-x)/sab*KKBLO-BLO)*TR*(-logab*(beta0-3d0*CF)+7d0*CA/3d0+5d0*beta0/3d0-8d0*CF))
c
c     Including correct multiplicity factor
      M2tmp(-2:0) = M2tmp(-2:0)*dble(%(proc_prefix_C_RV_qqx)s_den)/dble(%(proc_prefix_real)s_den)
...
c     compute collinear limit of sector function
      ...
      M2_C_RV_qqx(-2:0)=M2tmp(-2:0)*pref/sab*xj*extra*wc_nlo
c     apply flavour factor
      M2_C_RV_qqx(-2:0)=M2_C_RV_qqx(-2:0)*%(proc_prefix_real)s_fl_factor
c
c     plot
      wgtpl=-M2_C_RV_qqx(0)*wgt/nit*wgt_chan
      if(doplot)call histo_fill(xpb,xsb,nexternal-1,mapped_flavours,wgtpl)
c
c     sanity check
      if(abs(M2_C_RV_qqx(0)).ge.huge(1d0).or.isnan(M2_C_RV_qqx(0)))then
         write(77,*)'Exception caught in M2_C_RV_qqx',M2_C_RV_qqx(0)
         goto 999
      endif
c
      return

...
c     collinear double-soft limit C_(i,j) S_(i,j)
...
c     initial checks and label assignment
      if(lsec.eq.0)then
         ...
         elseif((ia.eq.jsec.and.ib.eq.ksec).or.(ia.eq.ksec.and.ib.eq.jsec)) then
            ...
            write(*,*)'Wrong indices 1 in M2_C_SS_qqx'
         ...
         elseif((ia.eq.ksec.and.ib.eq.lsec).or.(ia.eq.lsec.and.ib.eq.ksec)) then
            ...
            write(*,*)'Wrong indices 2 in M2_C_SS_qqx'
...
c     TODO:complete/check the sector
...
c
c     eikonal
c     See eq.C.36 in 2212.11190v2 TODO: complete the kernel (_y)
            M2tmp = -(TR*(1d0-2d0*zi*zj)/sij)!*bar_eik+qmunu[...]
            M2TMP = CCBLO*M2TMP
c     Including correct multiplicity factor
            M2tmp = M2tmp*dble(%(proc_prefix_Born)s_den)/dble(%(proc_prefix_rr)s_den)
            damp=1d0
            M2tmp=M2tmp*damp*xj/sab
            M2_C_SS_QQX=M2_C_SS_QQX+pref*M2tmp*WC_SS_NNLO*extra
c
c     plot
            wgtpl=-pref*M2tmp*WS_NNLO*extra*wgt/nit*wgt_chan

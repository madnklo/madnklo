...
c
 ...
 999  ierr=1
      ...
      double precision function M2_S_C_gg(ia,ib,ir,xs,xp,xsb,xpb,wgt,xj,nit,extra,wgt_chan,ierr)
c     soft-collinear limit SC_(ia,ib) * Wsoft-collinear
      use sectors4_module
      implicit none
      include 'nexternal.inc'
      INCLUDE 'coupl.inc'
      ...
      integer, parameter :: hel = - 1
      double precision alphas,alpha_qcd
c      double precision %(proc_prefix_S_C_gg)s_get_kkblo
c     set logical doplot
      logical doplot
      common/cdoplot/doplot
      double precision sCM
      ...
      integer %(proc_prefix_rr)s_den
      common/%(proc_prefix_rr)s_iden/%(proc_prefix_rr)s_den
      integer %(proc_prefix_S_C_gg)s_den
      common/%(proc_prefix_S_C_gg)s_iden/%(proc_prefix_S_C_gg)s_den
      integer isec,jsec,ksec,lsec,iref
      common/cpartindices/isec,jsec,ksec,lsec,iref
      integer asec,bsec,csec,dsec
      common/csecindices/asec,bsec,csec,dsec
...
c     checks
      if(.not.(leg_pdgs(ia).eq.21.and.leg_pdgs(ib).eq.21))then
         write(*,*)'Wrong pdgs in M2_S_C_gg',leg_pdgs(ia),leg_pdgs(ib)
         stop
      endif
      if(.not.(ia.eq.isec.and.ib.eq.jsec))then
         write(*,*)'Wrong indices in M2_S_C_gg',ia,ib,isec,jsec
         stop
      endif
c
c     possible cuts
...
c     safety check
      if(sab.le.0d0.or.sar+sbr.le.0d0.or.x.le.0d0.or.x.ge.1d0)then
         write(77,*)'Inaccuracy 1 in M2_S_C_gg',sab,sar+sbr,x
         goto 999
      endif
c
c     call Real as per eq. (C.10)
      call %(proc_prefix_S_C_gg)s_ME_ACCESSOR_HOOK(xpb,hel,alphas,ANS)
      RNLO = ANS(0)
c
      parent_leg = real_mapped_labels(ib)
c      KKRNLO = %(proc_prefix_S_C_gg)s_GET_KKBLO(parent_leg,xpb,kt)
c     TODO: improve ktmuktnuRmunu / kt^2
...
c     include prefactors
      M2_S_C_gg = M2_S_C_gg*dble(%(proc_prefix_S_C_gg)s_den)/dble(%(proc_prefix_rr)s_den)*%(proc_prefix_rr)s_fl_factor*damp*pref/sab*xj*extra
c
c     plot
      wgtpl=-M2_S_C_gg*wgt/nit*wgt_chan
      wgts=wgtpl
c      if(doplot)call histo_fill(xpb,xsb,nexternal-1,real_leg_pdgs,wgtpl)
      if(doplot)call analysis_fill(xpb,xsb,nexternal-1,real_leg_pdgs,wgts)
c
c     sanity check
      if(abs(M2_S_C_gg).ge.huge(1d0).or.isnan(M2_S_C_gg))then
         write(77,*)'Exception caught in M2_S_C_gg',M2_S_C_gg
         goto 999
      endif
c
      return

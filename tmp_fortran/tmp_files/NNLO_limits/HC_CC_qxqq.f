      ...
      double precision function M2_HC_CC_qxqq(i,j,k,r,xs,xp,xsb,xpb,xsbb,xpbb,wgt,xj,xjb,nit,extra,wgt_chan,ierr)
...
c     check flavour match
      ...
      if(.not.(flavourmatch))then
        write(*,*) 'Flavour mismatch in M2_HC_CC_qxqq', leg_PDGs(i),leg_PDGs(j),leg_PDGs(k)
        stop 1
      endif
c
c     possible cuts
...
c
      ...
      if(sbjk.lt.0d0.or.sbjr.lt.0d0.or.sbkr.lt.0d0) then
        write(77,*)'Inaccuracy 2 in M2_HC_CC_qxqq',sbjk,sbjr,sbkr
        goto 999
      endif
      zbj = sbjr/(sbjr+sbkr)
      zbk = 1d0-zbj
...
c     include correct multiplicity and flavour factors
      ...
      M2_HC_CC_qxqq = M2tmp*pref*xj*extra
      if(test_sector_function) M2_HC_CC_qxqq = wc_nlo*wcbar_nlo
c
c     plot
      wgtpl=-M2_HC_CC_qxqq*wgt/nit*wgt_chan
      wgts=wgtpl
...
c
c     sanity check
      if(abs(M2_HC_CC_qxqq).ge.huge(1d0).or.isnan(M2_HC_CC_qxqq))then
         write(77,*)'Exception caught in M2_HC_CC_qxqq',M2_HC_CC_qxqq
         goto 999
      endif
c
      return

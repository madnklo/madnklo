      ...
      double precision function M2_HC_HCC_qxqq(i,j,k,r,xs,xp,xsb,xpb,xsbb,xpbb,wgt,xj,xjb,nit,extra,wgt_chan,ierr)
...
c     initial checks and label assignment
      if(lsec.eq.0)then
         ...
         elseif((i.eq.jsec.and.j.eq.ksec).or.(i.eq.ksec.and.j.eq.jsec)) then
            ...
            write(*,*)'Wrong indices 1 in M2_HC_HCC_qxqq'
         ...
         elseif((i.eq.ksec.and.j.eq.lsec).or.(i.eq.lsec.and.j.eq.ksec)) then
            ...
            write(*,*)'Wrong indices 2 in M2_HC_HCC_qxqq'
...
c     safety check
      ...
      IF(SIJ.LE.0D0.OR.SIR.LE.0d0.OR.SJR.LE.0D0)THEN
        WRITE(77,*)'Inaccuracy 1 in M2_HC_HCC_qxqq',SIJ,SIR,SJR
        GOTO 999
      ENDIF
      zi   = sir/(sir+sjr)
      zj   = 1d0-zi
...
c
      M2_HC_HCC_qxqq=M2tmp*pref*xj*extra
c     apply flavour factor
      M2_HC_HCC_qxqq=M2_HC_HCC_qxqq*%(proc_prefix_rr)s_fl_factor
c
c     plot
      wgtpl=-M2_HC_HCC_qxqq*wgt/nit*wgt_chan
      if(doplot)call histo_fill(xpbbsave,xsbb,nexternal-2,BORN_LEG_PDGS,wgtpl)
c
c     sanity check
      if(abs(M2_HC_HCC_qxqq).ge.huge(1d0).or.isnan(M2_HC_HCC_qxqq))then
         write(77,*)'Exception caught in M2_HC_HCC_qxqq',M2_HC_HCC_qxqq
         goto 999
      endif
c
      return

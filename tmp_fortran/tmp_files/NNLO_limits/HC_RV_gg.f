      ...
      SUBROUTINE SUB_M2_HC_RV_gg(ia,ib,ir,xs,xp,xsb,xpb,wgt,xj,nit,extra,wgt_chan,ierr,ret)
...
c     for sector (ia,ib)
      ...
      double precision M2_C_gg(-2:0),M2_C_gg_0,M2_SC_gg(-2:0)
      ...
      double precision %(proc_prefix_HC_RV_gg)s_GET_KKBLO
      double precision %(proc_prefix_HC_RV_gg)s_GET_KKVLO
...
c     set logical doplot
      ...
      integer %(proc_prefix_HC_RV_gg)s_den
      common/%(proc_prefix_HC_RV_gg)s_iden/%(proc_prefix_HC_RV_gg)s_den
...
c     checks
      if(leg_pdgs(ia).ne.21.or.leg_pdgs(ib).ne.21.or.underlying_leg_pdgs(mapped_labels(ib)).ne.21)then
         write(*,*)'Wrong pdgs in M2_HC_RV_gg',leg_pdgs(ia),leg_pdgs(ib),mapped_labels(ib)
         stop
      endif
      if(.not.(ia.eq.isec.and.ib.eq.jsec.and.ir.eq.iref))then
         write(*,*)'Wrong indices in M2_HC_RV_gg',ia,ib,ir,isec,jsec,iref
         stop
      endif
c
c     possible cuts
...
c     safety check
      if(sab.le.0d0.or.sar+sbr.le.0d0.or.x.le.0d0.or.x.ge.1d0)then
         write(77,*)'Inaccuracy 1 in M2_HC_RV_gg',sab,sar+sbr,x
         goto 999
      endif
c
...
c      call %(proc_prefix_HC_RV_gg)s_ME_ACCESSOR_HOOK(xpb,hel,alphas,ANS)
      BLO = 0d0 !ANS(0)
      KKBLO = 0d0 !%(proc_prefix_HC_RV_gg)s_GET_KKBLO(parent_leg,xpb,kt)
c     call Virtual and spin-correlated Virtual
c      call %(proc_prefix_HC_RV_gg)s_ME_ACCESSOR_HOOK(xpb,hel,alphas,ANS)
      VLO = 0d0 !ANS(0)
      KKVLO = 0d0 !%(proc_prefix_HC_RV_gg)s_GET_KKVLO(parent_leg,xpb,kt)
c
      M2_C_GG_0 = CA*2d0*(2d0/sab*KKBLO+x/(1d0-x)*(1d0+1d0-x**alpha)*BLO+(1d0-x)/x*(1d0+1d0-(1d0-x)**alpha)*BLO)
      M2_C_GG(-2:0) = M2_C_GG(-2:0) + CA*2d0*(2d0/sab*KKVLO+x/(1d0-x)*(1d0+1d0-x**alpha)*VLO+(1d0-x)/x*(1d0+1d0-(1d0-x)**alpha)*VLO)
      M2_C_GG(-2) = M2_C_GG(-2) + alphas/2d0/pi*M2_C_GG_0*(-CA)
      M2_C_GG(-1) = M2_C_GG(-1) + alphas/2d0/pi*M2_C_GG_0*(CA*(logab+log(x*(1d0-x)))-beta0/2d0)
      M2_C_GG( 0) = M2_C_GG( 0) + alphas/2d0/pi*(M2_C_GG_0*CA*((7d0*zeta2-logab**2)/2d0-logab*log(x*(1d0-x))+ddilog(-(1d0-x)/x)+ddilog(-x/(1d0-x)))+1d0/sab*KKBLO*CA*(3d0*CA-beta0))
c
      M2_SC_gg(-2:0) = M2_SC_gg(-2:0)+2d0*CA*(EIK0*VLO(-2:0)-alphas/2d0/pi*EIK1(-2:0)*BLO)
      M2_SC_gg(-1)   = M2_SC_gg(-1)-2d0*CA*alphas/2d0/pi*beta0/2d0*EIK0*BLO
c
...
c     account for different damping factors according to recoiler position (ir)
      ...
      ret = M2_C_gg-M2_SC_gg
c     include prefactors
      ret = ret *dble(%(proc_prefix_HC_RV_gg)s_den)/dble(%(proc_prefix_real)s_den)*%(proc_prefix_real)s_fl_factor*damp*pref/sab*xj*extra
c
c     plot
...
c     sanity check
      if(abs(ret(0)).ge.huge(1d0).or.isnan(ret(0)))then
         write(77,*)'Exception caught in M2_HC_RV_gg',ret(0)
         goto 999
      endif
c
      return

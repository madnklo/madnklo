      ...
      double precision function M2_HC_SS_qqx_CC_qxqqp(i,j,k,r,xs,xp,xsb,xpb,xsbb,xpbb,wgt,xj,xjb,nit,extra,wgt_chan,ierr)
...
c
c     initialise
      M2_HC_SS_qqx_CC_qxqqp=0d0
      M2tmp=0d0
      ierr=0
c
c     check sector topology (only appears in ijjk)
      if(bsec.ne.csec) then
        write (*,*) 'Wrong topology in M2_HC_SS_qqx_CC_qxqqp',asec,bsec,csec,dsec
        stop 1
      endif
c
c     check flavour match
      ...
      if(.not.(flavourmatch))then
        write(*,*) 'Flavour mismatch in M2_HC_SS_qqx_CC_qxqqp', leg_PDGs(i),leg_PDGs(j),leg_PDGs(k)
        stop 1
      endif
c
c     possible cuts
...
c     safety checks
      if(sij.lt.0d0.or.sir.lt.0d0.or.sjr.lt.0d0)then
        write(77,*)'Inaccuracy 1 in M2_HC_SS_qqx_CC_qxqqp',sij,sir,sjr
        goto 999
      endif
      zi = sir/(sir+sjr)
      zj = 1d0-zi
      ...
      if(sbjk.lt.0d0.or.sbjr.lt.0d0.or.sbkr.lt.0d0) then
         write(77,*)'Inaccuracy 2 in M2_HC_SS_qqx_CC_qxqqp',sbjk,sbjr,sbkr
         goto 999
      endif
      zbj = sbjr/(sbjr+sbkr)
      zbk = 1d0-zbj
...
c     Including correct multiplicity factor
      ...
      M2_HC_SS_qqx_CC_qxqqp=M2tmp*pref*xj*extra
      if(test_sector_function) M2_HC_SS_qqx_CC_qxqqp = wc_nlo
c
c     plot
      wgtpl=-M2_HC_SS_qqx_CC_qxqqp*wgt/nit*wgt_chan
      wgts=wgtpl
...
c
c     sanity check
      if(abs(M2_HC_SS_qqx_CC_qxqqp).ge.huge(1d0).or.isnan(M2_HC_SS_qqx_CC_qxqqp)) then
         write(77,*)'Exception caught in M2_HC_SS_qqx_CC_qxqqp', M2_HC_SS_qqx_CC_qxqqp
         goto 999
      endif
c
      return
